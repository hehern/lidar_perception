...
#include "lidar_perception/common/graph_util.h"
#include "lidar_perception/hungarian/hungarian_bigraph_matcher.h"
...
void HungarianMatcher::Match(
  ...
  std::cout << "HungarianMatcher: partition graph into " << track_components.size()
         << " sub-graphs." << std::endl;
...
void HungarianMatcher::ComputeAssociateMatrix(
  ...
  for (size_t i = 0; i < tracks.size(); ++i) {
    for (size_t j = 0; j < new_objects.size(); ++j) {
      (*association_mat)(i, j) = TrackObjectDistance::ComputeDistance(    //计算新出现的object和已跟踪对象之间的距离
