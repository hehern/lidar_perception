...
bool KalmanFilter::InitParams(const double measurement_noise,
  ...
  if (measurement_noise < 0) {
    std::cout << "invalid measurement noise of KalmanFilter!" << std::endl;
    return false;
  }
  if (initial_velocity_noise < 0) {
    std::cout << "invalid initial velocity noise of KalmanFilter!" << std::endl;
    return false;
  }
  if (xy_propagation_noise < 0) {
    std::cout << "invalid xy propagation noise of KalmanFilter!" << std::endl;
    return false;
  }
  if (z_propagation_noise < 0) {
    std::cout << "invalid z propagation noise of KalmanFilter!" << std::endl;
    return false;
  }
  ...
  std::cout << "measurement noise of KalmanFilter is " << s_measurement_noise_ << std::endl;
  std::cout << "initial velocity noise of KalmanFilter is "
        << s_initial_velocity_noise_ << std::endl;
  std::cout << "propagation noise of KalmanFilter is\n" << s_propagation_noise_ << std::endl;
  return true;
}
...
void KalmanFilter::UpdateWithObject(
  ...
  if (time_diff <= DBL_EPSILON) {
    std::cout << "Time diff is too limited to updating KalmanFilter!" << std::endl;
    return;
  }
  // A. Compute update quality if needed
  if (s_use_adaptive_) {
    ComputeUpdateQuality(new_object, old_object);//计算更新力度(更新评分)
  } else {
    update_quality_ = 1.0;
  }
  ...
  Eigen::Vector3f measured_velocity =
      ComputeMeasuredVelocity(new_object, old_object, time_diff);//计算速度

  // debug: compute measured acceleration
  Eigen::Vector3f measured_acceleration =
      ComputeMeasuredAcceleration(measured_velocity, time_diff);//计算加速度

  // C. Update model
  UpdateVelocity(measured_anchor_point, measured_velocity, time_diff);//更新卡尔曼滤波模型中速度的最优值(估算最优速度)
  UpdateAcceleration(measured_acceleration);                          //更新加速度
...
//计算当前时刻的速度，相当于卡尔曼滤波中的观测量，共有三个速度：
Eigen::VectorXf KalmanFilter::ComputeMeasuredVelocity(
  ...
  Eigen::Vector3f measured_anchor_point_velocity =
      ComputeMeasuredAnchorPointVelocity(new_object, old_object, time_diff);//anchor point移动速度
  // Observation II: bbox center velocity measurement
  Eigen::Vector3f measured_bbox_center_velocity =
      ComputeMeasuredBboxCenterVelocity(new_object, old_object, time_diff);//计算标定框中心移动速度
  // Observation III: bbox corner velocity measurement
  Eigen::Vector3f measured_bbox_corner_velocity =
      ComputeMeasuredBboxCornerVelocity(new_object, old_object, time_diff);//计算标定框四个角点的速度
  ...
  Eigen::Vector3f measured_velocity =
      SelectMeasuredVelocity(measured_candidates);//选择增益最小的速度，增益＝当前速度减去上时刻最优速度（belief_velocity_）
...
Eigen::VectorXf KalmanFilter::ComputeMeasuredBboxCenterVelocity(
  ...
  Eigen::Vector3f project_dir =
      new_object->anchor_point - old_object->anchor_point;
  if (measured_bbox_center_velocity.dot(project_dir) <= 0) {//标定框中心速度和点云重心速度进行点乘，如果小于０，意味着夹角大于９０度，即方向相反
    measured_bbox_center_velocity = Eigen::Vector3f::Zero();//此时表示有干扰，中心速度置为０
  }
...
Eigen::Vector3f KalmanFilter::SelectMeasuredVelocityAccordingMotionConsistency(
  ...
  for (size_t i = 1; i < candidates.size(); ++i) {//查找candidate_velocity_gain最小值
...
void KalmanFilter::UpdateVelocity(const Eigen::VectorXf& measured_anchor_point,//卡尔曼速度更新
  ...
  velocity_covariance_ =
      (Eigen::Matrix3d::Identity() - mat_k * mat_c) * velocity_covariance_;//得到最优协方差矩阵
}
